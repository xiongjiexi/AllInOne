// Script Runner 配置文件加载：系统对话框选 YAML + 读取 + 解析
...
/** 解析 YAML 字符串为 ScriptRunnerConfig */
export function parseConfigYAML(text: string): ScriptRunnerConfig {
  const obj = yaml.load(text)
  if (!obj || typeof obj !== 'object') {
    throw new Error('配置文件为空或非对象')
  }
  return obj as ScriptRunnerConfig
}

/** 加载配置文件：读取 → 解析 → 校验 → 与默认配置合并 */
export async function loadConfigFile(path: string): Promise<ScriptRunnerConfig> {
  let text: string
  try {
