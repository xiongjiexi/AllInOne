...
export type ToolId = 'checklist' | 'gitfast' | 'scripts' | 'review' | 'textutils' | 'notes'
...
function detectInitial(): ToolId {
  ...
  if (saved === 'checklist' || saved === 'gitfast' || saved === 'scripts' || saved === 'review' || saved === 'textutils' || saved === 'notes') return saved
  return 'checklist'
}
